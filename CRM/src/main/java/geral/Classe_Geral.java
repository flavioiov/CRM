				...
				throws SQLException
		{
			try {
               ...
               SQL ="SELECT COLUMN_NAME,COLUMN_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '"+schema+"' AND TABLE_NAME ='"+nome_tabela+"'";
	     			...
	     			/*SQL="SELECT attrs.attname, Type";
					SQL=SQL+" FROM (	SELECT c.oid, n.nspname, c.relname	FROM pg_catalog.pg_class   c";
					SQL=SQL+" LEFT JOIN pg_catalog.pg_namespace n ON n.oid = c.relnamespace";
					SQL=SQL+" WHERE pg_catalog.pg_table_is_visible(c.oid)   ) rel";
					SQL=SQL+" JOIN (";
					SQL=SQL+" SELECT a.attname, a.attrelid, pg_catalog.format_type(a.atttypid,   a.atttypmod) as Type,";
					SQL=SQL+" (SELECT substring(d.adsrc for 128) FROM pg_catalog.pg_attrdef   d";
					SQL=SQL+" WHERE d.adrelid = a.attrelid AND d.adnum = a.attnum AND a.atthasdef) as Default,   a.attnotnull, a.attnum";
					SQL=SQL+" FROM pg_catalog.pg_attribute a WHERE a.attnum > 0   AND NOT a.attisdropped ) attrs";
					SQL=SQL+" ON (attrs.attrelid = rel.oid )";
					SQL=SQL+" WHERE relname = '"+nome_tabela+"' ORDER BY attrs.attnum";
			*/
		...
		try {
			...
			{			 	
			 	    ResultSet rs1;  
	  				...
	  				while (rs1.next())
					{
						String nome=rs1.getString("COLUMN_NAME");
						String tipo=rs1.getString("COLUMN_TYPE");
      ...
      try
        {
         ...
         Conexao conf = new Conexao(); 
           ...
           PreparedStatement prepared_statement;
		   String sql;
            sql = "select * from "+nome_tabela+" order by 2";
     		st = connection.createStatement();
			rs = st.executeQuery(sql);     
